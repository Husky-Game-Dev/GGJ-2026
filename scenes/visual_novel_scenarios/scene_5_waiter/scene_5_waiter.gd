...
func run_scenario() -> void:
	...
	await character_speak(waiter, [ "But don’t you see that this doesn't apply anymore? Everyone else agrees." ])
	await character_speak(waiter, [ "I guess… but the rulebook states–" ])
	await character_speak(waiter, [ "There has to be a rule you’ve been wanting to break." ])
