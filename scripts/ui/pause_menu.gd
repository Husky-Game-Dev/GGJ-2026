...
func _input(event: InputEvent) -> void:
	if menu_open && (event.is_action(&"pause") || event.is_action(&"ui_cancel")) && event.is_pressed():
...
func _on_input_state_changed(old_state: InputManager.InputState, new_state: InputManager.InputState) -> void:
	if old_state == InputManager.InputState.PAUSE_MENU:
		close_menu()
	if new_state == InputManager.InputState.PAUSE_MENU:
		open_menu()
